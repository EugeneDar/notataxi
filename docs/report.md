...
Сервис получает данные из источников, при этом используя требуемые в ADR механизмы защиты от отказов сервисов:
1. `config` использует кэш с ttl = 1 минута: [ссылка на код](https://github.com/EugeneDar/notataxi/blob/a6df93d1470eccd43b6d0d893189a45a16ea3705/internal/sources/grpc/sources.go#L98-L105)
2. `zone` использует кэш с ttl = 10 минут: [ссылка на код](https://github.com/EugeneDar/notataxi/blob/a6df93d1470eccd43b6d0d893189a45a16ea3705/internal/sources/grpc/sources.go#L115-L122)
3. `toll-roads` использует кэш + fallback (0), если кэш пустой: [ссылка на код](https://github.com/EugeneDar/notataxi/blob/a6df93d1470eccd43b6d0d893189a45a16ea3705/internal/sources/grpc/sources.go#L124-L132)
4. `executor` использует фоллбэк-источник: [ссылка на код](https://github.com/EugeneDar/notataxi/blob/a6df93d1470eccd43b6d0d893189a45a16ea3705/internal/sources/grpc/sources.go#L139-L146)
5. `order` -- критичный источник, поэтому для него никакого механизма нет.

### Результаты нагрузочного тестирования:

Обстрел микросервиса источников. Сервис держит 80 rps (столько позволила домашняя сеть при обстреле с компьютера).
![Обстрел микросервиса источников](sources_service_load_test.jpg "Обстрел микросервиса источников")

Обстрел микросервиса источников. Сервис держит 300 rps. Из этого можно сделать вывод, что и 100 rps микросервис источников держит.
![Обстрел микросервиса заказов](orders_service_load_test.jpg "Обстрел микросервиса заказов")

Как видно из графиков, время ответа не растет во время обстрела.

### Хранение данных

В Managed service for PostgreSQL БД может занимать как минимум до 2ТБ, а нам на 20 лет нужно 1204Гб, хватит (https://yandex.cloud/ru/docs/managed-postgresql/concepts/limits).
