...
func AssignOrderRequestHandler(c *gin.Context) {
	...
	if orderId == "" || executorId == "" || zoneId == "" {
		c.JSON(400, gin.H{"error": "Missing parameters, please provide order_id, executor_id and zone_id"})
		return
	}
...
func AcquireOrderRequestHandler(c *gin.Context) {
	executorId := c.Query("executor_id")
	if executorId == "" {
		c.JSON(400, gin.H{"error": "Missing parameters, please provide executor_id"})
		return
	}
	...
	c.JSON(200, gin.H{"order_profile": orderProfile})
...
func CancelOrderRequestHandler(c *gin.Context) {
	orderId := c.Query("order_id")
	if orderId == "" {
		c.JSON(400, gin.H{"error": "Missing parameters, please provide order_id"})
		return
	}
	...
	if !found {
		c.JSON(200, gin.H{"message": fmt.Sprintf("AssignedOrder with OrderId %s does not exist or has already been canceled\n", orderId)})
